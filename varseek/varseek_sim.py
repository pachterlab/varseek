...
def sim(
    ...
    **kwargs,
    ...
    if sample_type == "m":
        ...
        new_column_names = ["number_of_reads_wt", "list_of_read_starting_indices_wt", "any_noisy_reads_wt", "noisy_read_indices_wt"]
    else:
        sampled_reference_df["included_in_synthetic_reads_mutant"] = True
        sampled_reference_df["included_in_synthetic_reads_wt"] = True
        new_column_names = [
            ...
            if number_of_reads_per_sample == "all":  # sample all reads from sample_type (wt and/or mutant)
                ...
                if with_replacement:
                    read_start_indices_mutant = random.choices(
                        ...
                        k=min(valid_starting_index_max_mutant, number_of_reads_per_sample_m),
                    )
                else:
                    read_start_indices_mutant = random.sample(
                        range(valid_starting_index_max_mutant),
                        min(valid_starting_index_max_mutant, number_of_reads_per_sample_m),
                    )
                ...
                for selected_sequence, selected_strand in mutant_sequence_list:
                    for i in read_start_indices_mutant:
                        ...
                        if add_noise:
                            ...
                            sequence_chunk = introduce_sequencing_errors(sequence_chunk, error_rate=error_rate, max_errors=max_errors)
