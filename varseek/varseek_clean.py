...
from varseek.utils import plot_items_descending_order, plot_scree, plot_loading_contributions, find_resolution_for_target_clusters, plot_contingency_table, plot_knn_tissue_frequencies, plot_ascending_bar_plot_of_cluster_distances, plot_jaccard_bar_plot, plot_knee_plot, increment_adata_based_on_dlist_fns, decrement_adata_matrix_when_split_by_Ns_or_running_paired_end_in_single_end_mode, remove_adata_columns

def clean(adata_path, adata_output_path = None, output_figures_dir = None, id_to_header_csv = None, mutation_metadata_df = None, mutation_metadata_df_columns = None, minimum_count_filter = None, use_binary_matrix = False, drop_zero_columns = False, assay = "bulk", filter_cells_by_min_counts = None, filter_cells_by_min_genes = None, filter_genes_by_min_cells = None, filter_cells_by_max_mt_content = None, doublet_detection = None, remove_doublets = None, do_cpm_normalization = None, adata_path_normal_genome = None, split_reads_by_Ns = False, dlist_file = None, mcrs_id_column = "mcrs_id", verbose=False, **kwargs):
    ...
    if False:  #* erase once tested
        ...
        if dlist_file:
            # TODO: test this
            adata = increment_adata_based_on_dlist_fns(adata = adata, mcrs_fasta = mcrs_fasta, dlist_fasta = dlist_fasta, kb_count_out = kb_count_out, index = mcrs_index, t2g = mcrs_t2g, fastq = data_fastq, newer_kallisto = newer_kallisto, k=k, mm=mm, bustools=bustools)

    # set all count values below minimum_count_filter to 0
    if minimum_count_filter:
        adata.X.multiply(adata.X >= minimum_count_filter)
        ...
        if assay == "sc":
            if filter_cells_by_min_counts:
                if type(filter_cells_by_min_counts) != int:  # ie True for automatic
                    ...
                    knee_locator = KneeLocator(range(len(umi_counts_sorted)), umi_counts_sorted, curve='convex', direction='decreasing')
                    filter_cells_by_min_counts = umi_counts_sorted[knee_locator.knee]
                    plot_knee_plot(umi_counts_sorted=umi_counts_sorted, knee_locator=knee_locator, min_counts_assessed_by_knee_plot=filter_cells_by_min_counts, output_file = f"{output_figures_dir}/knee_plot.png")
            ...
            if filter_cells_by_max_mt_content:
                ...
                sc.pp.calculate_qc_metrics(adata_normal_genome, qc_vars=["mt"], percent_top=None, log1p=False, inplace=True)
    ...
    if mcrs_id_set_to_exclusively_keep:
        adata = remove_adata_columns(adata, values_of_interest = mcrs_id_set_to_exclusively_keep, operation = "keep", var_column_name = "mcrs_id")
    if mcrs_id_set_to_exclude:
        adata = remove_adata_columns(adata, values_of_interest = mcrs_id_set_to_exclude, operation = "exclude", var_column_name = "mcrs_id")
    if transcript_set_to_exclusively_keep:
        adata = remove_adata_columns(adata, values_of_interest = transcript_set_to_exclusively_keep, operation = "keep", var_column_name = "seq_ID")
    if transcript_set_to_exclude:
        adata = remove_adata_columns(adata, values_of_interest = transcript_set_to_exclude, operation = "exclude", var_column_name = "seq_ID")
    if gene_set_to_exclusively_keep:
        adata = remove_adata_columns(adata, values_of_interest = gene_set_to_exclusively_keep, operation = "keep", var_column_name = "gene_name")
