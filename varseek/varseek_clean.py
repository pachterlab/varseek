...
def validate_input_clean(params_dict):
    ...
    strand_bias_end_valid_values = {"5p", "3p"}
...
def clean(
    ...
    **kwargs,
    ...
    - variants                              (str): The variants parameter from vk ref. Only needed if using downloaded reference files from vk ref and wanting to save an output VCF file. Default: None.
    - sequences                             (str): The sequences parameter from vk ref. Only needed if using downloaded reference files from vk ref and wanting to save an output VCF file. Default: None.
    ...
    if account_for_strand_bias:
        adata = remove_variants_from_adata_for_stranded_technologies(adata=adata, strand_bias_end=strand_bias_end, read_length=read_length)
