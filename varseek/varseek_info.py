...
def add_some_mutation_information_when_cdna_and_genome_combined(df, columns_to_change):
    for column in columns_to_change:
        ...
        df.loc[df["source"] == "cdna", f"{column}_cdna"] = df.loc[
            df["source"] == "cdna", column
        ]
        df.loc[df["source"] == "genome", f"{column}_genome"] = df.loc[
            df["source"] == "genome", column
        ]
    ...
    for id_val, group in grouped:
        for column in columns_to_change:
            ...
            cdna_info_value = group.loc[
                group["source"] == "cdna", f"{column}_cdna"
            ].values
            genome_info_value = group.loc[
                group["source"] == "genome", f"{column}_genome"
            ].values
...
def add_mutation_information(
    mutation_metadata_df, mutation_column="mutation", mcrs_source="cdna"
):
    mutation_metadata_df[["nucleotide_positions", "actual_mutation"]] = (
        mutation_metadata_df[mutation_column].str.extract(mutation_pattern)
    )

    split_positions = mutation_metadata_df["nucleotide_positions"].str.split(
        "_", expand=True
    )
    mutation_metadata_df[f"start_mutation_position"] = split_positions[0]

    if split_positions.shape[1] > 1:
        mutation_metadata_df[f"end_mutation_position"] = split_positions[1].fillna(
            split_positions[0]
        )
    else:
        mutation_metadata_df[f"end_mutation_position"] = mutation_metadata_df[
            "start_mutation_position"
        ]

    mutation_metadata_df[["start_mutation_position", "end_mutation_position"]] = (
    ...
    if mcrs_source is not None:
        mutation_metadata_df[f"nucleotide_positions_{mcrs_source}"] = (
            mutation_metadata_df[f"nucleotide_positions"]
        )
        mutation_metadata_df[f"actual_mutation_{mcrs_source}"] = mutation_metadata_df[
            f"actual_mutation"
        ]
        mutation_metadata_df[f"start_mutation_position_{mcrs_source}"] = (
            mutation_metadata_df[f"start_mutation_position"]
        )
        mutation_metadata_df[f"end_mutation_position_{mcrs_source}"] = (
            mutation_metadata_df[f"end_mutation_position"]
        )
...
def info(
    ...
    **kwargs,
    ...
    if mutation_metadata_df_out_path is None:
        mutation_metadata_df_out_path = os.path.join(
            out_dir_notebook, "mutation_metadata_df_vk_info.csv"
        )
    ...
    if id_to_header_csv is not None:
        ...
        swap_ids_for_headers_in_fasta(
            mutations, id_to_header_csv, out_fasta=temp_header_fa
        )
    else:
    ...
    if updated_df is None:  # does not support concatenated cdna and genome
        ...
        mutation_metadata_df = pd.DataFrame(
            data, columns=[mcrs_id_column, "mcrs_sequence"]
        )

        if id_to_header_dict is not None:
            mutation_metadata_df["mcrs_header"] = mutation_metadata_df[
                mcrs_id_column
            ].map(id_to_header_dict)
        else:
            mutation_metadata_df["mcrs_header"] = mutation_metadata_df[mcrs_id_column]

        contains_enst = (
            mutation_metadata_df["mcrs_header"].iloc[0].find("ENST") != -1
        )  # TODO: this just differentiates cdna from genome based on searching for ENST, but there may be other ways to discern cDNA from genome
        ...
        mcrs_header_has_merged_values = (
            mutation_metadata_df["mcrs_header"]
            .apply(lambda x: isinstance(x, str) and ";" in x)
            .any()
        )

        if mcrs_header_has_merged_values:
            mutation_metadata_df_exploded = explode_df(
                mutation_metadata_df, columns_to_explode
            )
        else:
            mutation_metadata_df_exploded = mutation_metadata_df

        mutation_metadata_df_exploded[["seq_ID", "mutation"]] = (
            mutation_metadata_df_exploded["header"].str.split(":", expand=True)
        )

        mutation_metadata_df_exploded["seq_ID"] = mutation_metadata_df_exploded[
            "seq_ID"
        ].astype(str)

        mutation_metadata_df_exploded = add_mutation_information(
            mutation_metadata_df_exploded, mcrs_source=mcrs_source
        )
        ...
        columns_to_explode_extend_values = [
            ...
            if col not in [mcrs_id_column, "mcrs_header", "mcrs_sequence"]
            and col not in columns_to_explode
        ]
        ...
        if mcrs_source == "combined":
            mutation_metadata_df = (
                add_some_mutation_information_when_cdna_and_genome_combined(
                    mutation_metadata_df, columns_to_explode
                )
            )
        else:
            ...
            mutation_metadata_df[f"actual_mutation_{mcrs_source}"] = (
                mutation_metadata_df[f"actual_mutation"]
            )
            mutation_metadata_df[f"start_mutation_position_{mcrs_source}"] = (
                mutation_metadata_df[f"start_mutation_position"]
            )
        ...
        if (
            "chromosome" in mutation_metadata_df_exploded.columns
            and "mutation_genome" in mutation_metadata_df_exploded.columns
        ):
            mutation_metadata_df_exploded["header_genome"] = (
                mutation_metadata_df_exploded["chromosome"].astype(str)
                + ":"
                + mutation_metadata_df_exploded["mutation_genome"].astype(str)
            )

        if (
            "seq_ID" in mutation_metadata_df_exploded.columns
            and "mutation" in mutation_metadata_df_exploded.columns
        ):
            mutation_metadata_df_exploded["header_cdna"] = (
                mutation_metadata_df_exploded["seq_ID"].astype(str)
                + ":"
                + mutation_metadata_df_exploded["mutation"].astype(str)
            )

        if (
            "seq_ID" in mutation_metadata_df_exploded.columns
            and "mutation_cds" in mutation_metadata_df_exploded.columns
        ):
            mutation_metadata_df_exploded["header_cds"] = (
                mutation_metadata_df_exploded["seq_ID"].astype(str)
                + ":"
                + mutation_metadata_df_exploded["mutation_cds"].astype(str)
            )
        ...
        if (
            mutation_metadata_df_exploded["mcrs_source"].unique()[0] == "cdna"
            and mutation_genome_column in mutation_metadata_df_exploded
        ):
            mutation_metadata_df_exploded = add_mutation_information(
            ...
            mutation_metadata_df_exploded[f"nucleotide_positions"] = (
                mutation_metadata_df_exploded[f"nucleotide_positions_{mcrs_source}"]
            )
            mutation_metadata_df_exploded[f"actual_mutation"] = (
                mutation_metadata_df_exploded[f"actual_mutation_{mcrs_source}"]
            )
            mutation_metadata_df_exploded[f"start_mutation_position"] = (
                mutation_metadata_df_exploded[f"start_mutation_position_{mcrs_source}"]
            )
            mutation_metadata_df_exploded[f"end_mutation_position"] = (
                mutation_metadata_df_exploded[f"end_mutation_position_{mcrs_source}"]
            )

        if (
            mutation_metadata_df_exploded["mcrs_source"].unique()[0] == "genome"
            and mutation_cdna_column in mutation_metadata_df_exploded
        ):
            mutation_metadata_df_exploded = add_mutation_information(
                mutation_metadata_df_exploded,
    ...
    if mcrs_source == "genome" or mcrs_source == "combined":
        mutation_metadata_df_exploded = mutation_metadata_df_exploded.loc[
            ~(
                (mutation_metadata_df_exploded[mcrs_source_column] == "genome")
                & (
                    (pd.isna(mutation_metadata_df_exploded["chromosome"]))
                    | (
                        mutation_metadata_df_exploded["mutation_genome"].str.contains(
                            "g.nan", na=True
                        )
                    )
                )
    ...
    if (
        columns_to_include == "all"
        or "distance_to_nearest_splice_junction" in columns_to_include
    ):
        # Add metadata: distance to nearest splice junction
        try:
            logger.info("Computing distance to nearest splice junction")
            mutation_metadata_df_exploded, columns_to_explode = (
                compute_distance_to_closest_splice_junction(
                    mutation_metadata_df_exploded,
                    gtf,
                    columns_to_explode=columns_to_explode,
    ...
    if (
        ...
        or "header_with_gene_name" in columns_to_include
    ):
        total_genes_output_stat_file = (
            f"{output_stat_folder}/total_genes_and_transcripts.txt"
        )
        try:
            ...
            columns_not_successfully_added.extend(
                ["number_of_mutations_in_this_gene_total", "header_with_gene_name"]
            )
    ...
    if columns_to_include == "all" or (
        "nearby_mutations" in columns_to_include
        or "nearby_mutations_count" in columns_to_include
        or "has_a_nearby_mutation" in columns_to_include
    ):
        try:
    ...
    mutation_metadata_df[mcrs_id_column] = mutation_metadata_df[mcrs_id_column].astype(
        str
    )
    ...
    if columns_to_include == "all" or "mcrs_header_length" in columns_to_include:
        try:
            ...
            mutation_metadata_df["mcrs_header_length"] = mutation_metadata_df[
                "mcrs_header"
            ].str.len()
        except Exception as e:
    ...
    if columns_to_include == "all" or "mcrs_sequence_length" in columns_to_include:
        try:
            ...
            mutation_metadata_df["mcrs_sequence_length"] = mutation_metadata_df[
                "mcrs_sequence"
            ].str.len()
        except Exception as e:
    ...
    if "ensembl" in dlist_reference_source:
        reference_out_dir_sequences_dlist = (
            f"{reference_out_dir}/{dlist_reference_source}"
        )
    ...
    if columns_to_include == "all" or (
        "dlist" in columns_to_include
        or "number_of_alignments_to_normal_human_reference" in columns_to_include
        or "dlist_substring" in columns_to_include
        or "number_of_substring_matches_to_normal_human_reference" in columns_to_include
    ):
        try:
    ...
    if columns_to_include == "all" or (
        "pseudoaligned_to_human_reference" in columns_to_include
        or "pseudoaligned_to_human_reference_despite_not_truly_aligning"
        in columns_to_include
    ):
        ...
        try:
            ...
            logger.error(
                f"Error getting MCRSs that pseudoalign but aren't dlisted: {e}"
            )
    ...
    if columns_to_include == "all" or (
        ...
        in columns_to_include
        or "number_of_mcrs_items_with_overlapping_kmers_in_mcrs_reference"
        in columns_to_include
    ):
        try:
            ...
            mutation_metadata_df = mutation_metadata_df.merge(
                df_overlap, on=mcrs_id_column, how="left"
            )
            mutation_metadata_df["kmer_overlap_in_mcrs_reference"] = (
                mutation_metadata_df[
                    "number_of_kmers_with_overlap_to_other_mcrs_items_in_mcrs_reference"
                ].astype(bool)
            )
    ...
    if columns_to_include == "all" or (
        "longest_homopolymer_length" in columns_to_include
        or "longest_homopolymer" in columns_to_include
    ):
        try:
            ...
            (
                ...
                *mutation_metadata_df["mcrs_sequence"].apply(
                    lambda x: (
                        longest_homopolymer(x) if pd.notna(x) else (np.nan, np.nan)
                    )
                )
            )
        except Exception as e:
    ...
    if columns_to_include == "all" or (
        "num_distinct_triplets" in columns_to_include
        or "num_total_triplets" in columns_to_include
        or "triplet_complexity" in columns_to_include
    ):
        ...
        try:
            (
                ...
                *mutation_metadata_df["mcrs_sequence"].apply(
                    lambda x: (
                        triplet_stats(x) if pd.notna(x) else (np.nan, np.nan, np.nan)
                    )
                )
            )

            output_file_longest_homopolymer = (
                f"{output_plot_folder}/longest_homopolymer.png"
            )
            plot_histogram_of_nearby_mutations_7_5(
                mutation_metadata_df,
            ...
            output_file_triplet_complexity = (
                f"{output_plot_folder}/triplet_complexity.png"
            )
            plot_histogram_of_nearby_mutations_7_5(
                mutation_metadata_df,
                "triplet_complexity",
    ...
    if columns_to_include == "all" or "mcrs_mutation_type" in columns_to_include:
        try:
            ...
            mutation_metadata_df = add_mcrs_mutation_type(
                mutation_metadata_df, mut_column="mcrs_header"
            )
        except Exception as e:
    ...
    if columns_to_include == "all" or (
        "concatenated_headers_in_mcrs" in columns_to_include
        or "number_of_mutations_in_mcrs_header" in columns_to_include
    ):
        try:
            logger.info("Adding concatenated header info")
            mutation_metadata_df["concatenated_headers_in_mcrs"] = mutation_metadata_df[
                "mcrs_header"
            ].str.contains(";")
            mutation_metadata_df["number_of_mutations_in_mcrs_header"] = (
                mutation_metadata_df["mcrs_header"].str.count(";") + 1
            )
        except Exception as e:
    ...
    if columns_to_include == "all" or "mcrs_sequence_rc" in columns_to_include:
        try:
            ...
            mutation_metadata_df["mcrs_sequence_rc"] = mutation_metadata_df[
                "mcrs_sequence"
    ...
    if columns_to_include == "all" or (
        "entries_for_which_this_mcrs_is_substring" in columns_to_include
        or "entries_for_which_this_mcrs_is_superstring" in columns_to_include
        or "mcrs_is_substring" in columns_to_include
        or "mcrs_is_superstring" in columns_to_include
    ):
        ...
        try:
            ...
            substring_to_superstring_df, superstring_to_substring_df = (
                create_df_of_mcrs_to_self_headers(
            ...
            mutation_metadata_df[mcrs_id_column] = mutation_metadata_df[
                mcrs_id_column
            ].astype(str)
            mutation_metadata_df = mutation_metadata_df.merge(
                substring_to_superstring_df, on=mcrs_id_column, how="left"
            )
            mutation_metadata_df = mutation_metadata_df.merge(
                superstring_to_substring_df, on=mcrs_id_column, how="left"
            )

            mutation_metadata_df["mcrs_is_substring"] = (
                mutation_metadata_df["mcrs_is_substring"].fillna(False).astype(bool)
            )
            mutation_metadata_df["mcrs_is_superstring"] = (
                mutation_metadata_df["mcrs_is_superstring"].fillna(False).astype(bool)
            )
    ...
    if save_exploded_df:
        ...
        mutation_metadata_df_out_path_exploded = mutation_metadata_df_out_path.replace(
            ".csv", "_exploded.csv"
        )
        mutation_metadata_df_exploded.to_csv(
            mutation_metadata_df_out_path_exploded, index=False
        )
    ...
    if verbose:
        ...
        logger.info(
            f"Columns successfully added: {set(mutation_metadata_df.columns.tolist()) - set(columns_original)}"
        )
