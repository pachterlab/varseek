...
def fastqpp(
    ...
    if trim_edges_off_reads:
        ...
        rnaseq_fastq_files_list = trim_edges_off_reads_fastq_list(rnaseq_fastq_files=rnaseq_fastq_files_list, parity=parity, minimum_base_quality_trim_reads=minimum_base_quality_trim_reads, qualified_quality_phred=qualified_quality_phred, unqualified_percent_limit=unqualified_percent_limit, n_base_limit=n_base_limit, length_required=minimum_length, fastp = fastp)
    ...
    if replace_low_quality_bases_with_N:
        ...
        rnaseq_fastq_files_list = replace_low_quality_bases_with_N_list(rnaseq_fastq_files_quality_controlled=rnaseq_fastq_files_list, minimum_base_quality_replace_with_N=minimum_base_quality_replace_with_N, seqtk=seqtk)
        rnaseq_fastq_files_list_dict["replaced_with_N"] = rnaseq_fastq_files_list

    if split_reads_by_Ns:
        print("Splitting reads by Ns")
        rnaseq_fastq_files_list = split_reads_by_N_list(rnaseq_fastq_files_list, minimum_sequence_length=minimum_length, delete_original_files=delete_intermediate_files)
