...
#* VCRS
kb_count_vcrs_command = f"kb count -t 2 -k 51 --union -i {vcrs_index} -g {vcrs_t2g} -x {technology} --num --h5ad -o {kb_count_out_vcrs} "
if technology in {"BULK", "SMARTSEQ2"}:
    kb_count_vcrs_command += f"--parity {vcrs_parity} "
if mm:
    kb_count_vcrs_command += "--mm "
    kb_count_out_vcrs += "_mm"
...
start = time.time()
adata_adjusted_for_normal_genome = adjust_variant_adata_by_normal_gene_matrix(kb_count_vcrs_dir=kb_count_out_vcrs, kb_count_reference_genome_dir=kb_count_out_normal, fastq_file_list=fastqs, technology=technology, t2g_standard=normal_t2g, adata_output_path=None, mm=mm, parity=parity, bustools="/Users/joeyrich/miniconda3/envs/varseek/lib/python3.10/site-packages/kb_python/bins/darwin/m1/bustools/bustools", fastq_sorting_check_only=True, save_type="parquet", count_reads_that_dont_pseudoalign_to_reference_genome=True, variant_source="transcriptome")
adata_adjusted_for_normal_genome.write_h5ad(kb_count_out_vcrs + f"/adata_adjusted_for_normal_genome_with_logging_mm{mm}.h5ad")
end = time.time()
